...
![](_static/images/Coriolis-Web-Endpoints.png)
...
![](_static/images/Coriolis-Web-Create-Endpoint.png) Choose “**OpenStack**” and provide all the required data. Make also sure to set “**Allow Untrusted**” to “**Yes**” if your OpenStack API endpoint uses a HTTPS self signed certificate.

![](_static/images/Coriolis-Web-Create-OpenStack-Endpoint-2.png) Once done, click “**Save**”. This will create the endpoint and trigger a validation of the data by attempting a connection to the OpenStack API.

![](_static/images/Coriolis-Web-Validating-Endpoint.png)
...
![](_static/images/Coriolis-Web-Create-VMware-Endpoint.png)
...
![](_static/images/Coriolis-Web-New-Replica-1.png)

Click “**Next**” and select the source endpoint (**VMware**):

![](_static/images/Coriolis-Web-New-Replica-2.png)
...
![](_static/images/Coriolis-Web-New-replica-9-OpenStack.png) Click “**Finish**” and the replica will start executing: ![](_static/images/Coriolis-Web-New-replica-10-OpenStack.png) You can now click on the replica to see the execution details: ![](_static/images/Coriolis-Web-Replica-Execution-OpenStack.png) Once completed, the replica can be executed incrementally multiple times to update the replicated content (this is typically scheduled, e.g. hourly, daily, weekly, etc).

### Migrating the VM from the replica

From the replica details, you can click on “**Migrate Replica**” to get the VM(s) running on OpenStack: ![](_static/images/Coriolis-Web-Migrate-Replica-OpenStack.png)

You can click on “**View Migration Status**” to watch the migration progress. The "**migrations**" view can also be reached through the main menu (click on the top left icon). ![](_static/images/Coriolis-Web-Migrate-Replica-2-OpenStack.png) Once completed, the migration status will change accordingly:

![](_static/images/Corolis-Web-Migration-completed-OpenStack.png) Congratulations, your VM is now running and accessible from the OpenStack Horizon dashboard: ![](_static/images/Corolis-Web-Migration-completed-Horizon.png)
